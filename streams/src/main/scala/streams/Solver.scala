...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    (for {
      pairs <- b.legalNeighbors
    } yield (pairs._1, pairs._2 :: history)).to(LazyList)}
  ...
  def from(initial: LazyList[(Block, List[Move])], explored: Set[Block]): LazyList[(Block, List[Move])] = {
    if (initial.isEmpty) LazyList[(Block, List[Move])]()
      ...
      val tmp = (for {
        init_pair <- initial
        next_pa <- newNeighborsOnly(neighborsWithHistory(init_pair._1, init_pair._2), explored)
      } yield next_pa).to(Set)
      initial ++ from(tmp.to(LazyList), explored ++ (tmp map (pa => pa._1)))
    }
  }

  lazy val pathsFromStart: LazyList[(Block, List[Move])] = from(List((startBlock,List[Move]())).to(LazyList),Set.empty)
