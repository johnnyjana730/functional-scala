...
//  Decoder.fromPartialFunction { case Json.Bool(n) => (n) }
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(x) => {
        val item = x.map(decoder.decode)
        if (item.forall(_.isDefined)) Some(item.map(_.get))
        else Option.empty
      }
      case _  =>  Option.empty
    }
...
//  implicit val ContactsCodecsEncoder: Encoder[List[Person]] =
//    tmp_Encoder.listEncoder[Person](tmp_Person.personEncoder)
////    Encoder.fromFunction(as => Json.Arr(as.map(ObjectEncoder.field[String]("name")
////      .zip(ObjectEncoder.field[Int]("age"))
////      .transform[Person](user => (user.name, user.age)))))
//
//  /** The corresponding decoder for `Person` */
//  implicit val ContactsCodecsDecoder: Decoder[List[Person]] =
//    tmp_decoder.listDecoder[Person](tmp_Person.personDecoder)
